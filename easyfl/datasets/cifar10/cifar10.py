...
class Cifar10(BaseDataset):
    ...
    def download_raw_file_and_extract(self):
        ...
        self.train_data = {   #10000
            'x': train_set.data,
            'y': train_set.targets
        }
        self.test_data = { #50000
            'x': test_set.data,
            'y': test_set.targets
        }
    ...
    def preprocess(self):
        ...
        if self.is_ssl and (self.s_split_type=='dir' and (self.split_type=='dir')):
            logger.info("Start CIFAR10 data dirichlet simulation")
            ...
            s_train_data, u_train_data = split_s_and_u_dir(train_local_data, self.ssl_senario, self.num_labels_per_class, self.num_of_client)

...
from easyfl.datasets.dataset_util import TransformDataset, ImageDataset,TransformDataset_plus_idx,TensorDataset_plus_idx
...
class FederatedTensorDataset(FederatedDataset):
    ...
    def loader_plus_idx(self, batch_size, client_id=None, shuffle=True, seed=0, transform=None, drop_last=False):
        """Get dataset loader.
        ...
        transform = self.transform if transform is None else transform
        if transform is not None:
            dataset = TransformDataset_plus_idx(data_x,
                                       data_y,
                                       transform_x=transform,
                                       transform_y=self.target_transform)
        else:
            dataset = TensorDataset_plus_idx(data_x, data_y)
    ...
    def get_num_per_class(self,client_id):
        data=self.data[client_id]
        data_y=data['y'][:].tolist()
        num_class=len(list(set(data_y)))
        classes=list(set(data_y))
        num_per_classes={}
        for idx in classes:
            num_per_classes[idx]=data_y.count(int(idx))
